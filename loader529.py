...
fileDates = trainData['date']           #store dates
fileTypes = trainData['type']          #store Types
trainData=       trainData.drop('date',axis=1)    # remove date column
trainData =      trainData.drop ('type', axis =1) # remove type column
fileCols  = trainData.columns          # grab col names for re-insersion after normalization
...
scaler = StandardScaler().fit(trainData)     #0 mean 1, std deviation
trainData = scaler.transform(trainData)
print (trainData)
#'''''
trainData = pd.DataFrame(data=trainData, columns=fileCols)    # change normalized data back into pandas data frame, reinsert columns
...
trainData.plot(kind='density', subplots=True, sharex=False,layout=(3,4),)
#pyplot.show()
print (trainData)
pyplot.show()
